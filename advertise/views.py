        ...
        if author == '':
            if subject == "선택 없음":
                if region == "선택 없음":
                    ...
                    context = Post.objects.filter(region__area=region,
                                                  classifiedregion__classified_region=classified_region_name)
            elif region == "선택 없음":
            ...
            if subject == "선택 없음":
                if region == "선택 없음":
                    context = Post.objects.filter(
                        Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2))
                elif classified_region_name == "선택 없음":
                    context = Post.objects.filter(
                        Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2),
                        region__area=region)
                    ...
                    context = Post.objects.filter(
                        Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2),
                        region__area=region, classifiedregion__classified_region=classified_region_name)
            elif region == "선택 없음":
                context = Post.objects.filter(
                    Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2),
                    subject__subject_title=subject)
            elif classified_region_name == "선택 없음":
                context = Post.objects.filter(
                    Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2),
                    subject__subject_title=subject, region__area=region)
            else:
                context = Post.objects.filter(
                    Q(author__first_name__icontains=query_name1) or Q(author__first_name__icontains=query_name2),
                    subject__subject_title=subject, region__area=region,
